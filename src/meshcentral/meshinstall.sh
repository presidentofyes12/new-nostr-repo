...
DownloadAgent() {
  ...
  wget $url/meshagents?id=$machineid --no-check-certificate -O ./meshagent || curl -L -k --output ./meshagent $url/meshagents?id=$machineid

  # If it did not work, try again using http
  if [ $? != 0 ]
  then
    url=${url/"https://"/"http://"}
    wget $url/meshagents?id=$machineid -O ./meshagent || curl -L --output ./meshagent $url/meshagents?id=$machineid
  fi
  ...
  then
    ...
    chmod 755 ./meshagent
    wget $url/meshsettings?id=$meshid --no-check-certificate -O ./meshagent.msh || curl -L -k --output ./meshagent.msh $url/meshsettings?id=$meshid
