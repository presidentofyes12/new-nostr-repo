...
class IVCSItem(models.Model):
    ...
    sync_manager_id = fields.Many2one('ivcs.sync.manager', string='Sync Manager')
    ...
    def create(self, vals):
        if not vals.get('current_version_id'):
            version = self.env['ivcs.version'].create({
                'name': 'v1.0',
                'description': 'Initial version',
            })
            vals['current_version_id'] = version.id
    ...
    def create(self, vals):
        ...
        initial_version = self.env['ivcs.version'].create({
            'name': 'v1.0',
            'description': 'Initial version',
            'item_id': item.id,
        })
        item.current_version_id = initial_version.id
        item._initialize_repository()
        return item
    """
    ...
    def _create_commit(self, message, commit_hash):
        return self.env['ivcs.commit'].create({
            'item_id': self.id,
            'name': commit_hash,
            'message': message,
    ...
    def _create_initial_version(self, commit_hash):
        commit = self._create_commit("Initial commit", commit_hash)
        self.env['ivcs.version'].create({
            'name': 'v1.0',
            'description': 'Initial version',
            'item_id': self.id,
            'commit_id': commit.id,
